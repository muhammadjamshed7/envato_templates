...
export const coursesData = createAsyncThunk(
    ...
    async () => {
      const response = await fetch('https://eduker-server.onrender.com/courses')
...
export const singleCourse = createAsyncThunk(
    ...
    async (id = '622037c6ea7975d304b6b4ef') => {
      const response = await fetch(`https://eduker-server.onrender.com/courseDetails/${id}`)
...
export const coursesSlice = createSlice({
  ...
  reducers: {
    searchText: (state, {payload}) => {
      ...
      state.searchCoursesItems = state.allCourses.filter(course => course.title
